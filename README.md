...
# Nota : 
Es importante tener tu token ya que te lo pedira cada vez que subas un cambio en tus repositorios , o rama de Proyecto con los companeros .
...
"En caso de Eliminar un Archivo"-->  git rm "Nombre del archivo con su ruta" 

```


si quieres subir todos los cambios  en un solo comando (nota , no es recomendable ) usa el siguiente comando --->
```shell
git add *
```
...
`git pull` Te pedira el usuario de tu cuenta github y luego tu llave toket ( debes crear una si no posees.  ) 


# Paso 3 Trabajar en Proyectos con tu Equipo . 
...
"Por ejemplo, si deseas cambiar a la rama develop, debes escribir git checkout develop."
...
"Luego, puedes realizar cambios en los archivos del repositorio. Para guardar los cambios, debes hacer un commit utilizando el comando git commit."
...
"Esto descarga los cambios de la rama remota, pero no los fusiona con la rama local. Si deseas fusionar los cambios, debes utilizar el comando" git merge [nombre-remoto]/[nombre-de-rama].
...
# Descargar otras Ramas y Verificar el estado de la Ramaen Github 

``` bash
Para descargar todas las ramas de un repositorio remoto, debes utilizar el comando 

git fetch [nombre-remoto]. "Por ejemplo, si el nombre del repositorio remoto es origin, debes escribir git fetch origin. Esto descargará todas las ramas del repositorio remoto, pero no las fusionará con las ramas locales. "
...
"Por ejemplo, si deseas fusionar la rama remota feature con la rama local master, debes escribir git merge origin/feature."
...
git merge [nombre-remoto]/[nombre-de-rama] o git pull [nombre-remoto] [nombre-de-rama]. 

"El comando git pull es una forma abreviada de escribir git fetch y git merge en un solo comando"
